...
public class Main {
    ...
    public static String calc(String input) throws Exception {  //метод, который необходимо реализовать
        ...
        if (type_a1.equals("roma")) { //вывод результатов, если были римские числа
            if (rezult < 1) {
                throw new Exception(); //выброс исключения: ответ меньше 1 для римских чисел
            } else {
...
class Operations {
    ...
    public String digit(String x) { // проверка чисел на тип цифр, входящих в их состав
        String arabia = "123456789"; // строка арабских цифр
        ...
        if (x.length() < 5) { // проверка на римские цифры
